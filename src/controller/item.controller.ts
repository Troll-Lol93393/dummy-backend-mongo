...
export const createItem = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    const { itemCode, itemName, itemDesc, itemType, size, bom } = req.body;
...
export const updateItem = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    const { itemCode } = req.query;
    const { itemName, itemDesc, itemType, size, bom } = req.body;
    const item = await Item.findOne({ itemCode });
    if (!item) {
        throw new ApiError(404, "Item not found");
    }
    if (itemName !== undefined) item.itemName = itemName;
    if (itemDesc !== undefined) item.itemDesc = itemDesc;
    if (itemType !== undefined) item.itemType = itemType;
    if (size !== undefined) item.size = size;
    if (bom !== undefined) item.bom = bom;
    await item.save();
