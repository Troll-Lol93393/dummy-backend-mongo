...
export function generateCommercialOfferPdf(data: CommercialOfferData): PassThrough {
...
function drawLetterhead(doc: PDFKit.PDFDocument, _data: CommercialOfferData): void {
    ...
    doc.font("Helvetica-Bold").fontSize(24).fillColor(COLORS.headerText)
        .text("SHETH ENGINEERING", leftX + 20, headerY + 12, { width: pageWidth - 40 });

    doc.font("Helvetica").fontSize(9).fillColor("#8faabe")
        .text("Precision Engineering & Manufacturing Solutions", leftX + 20, headerY + 42, { width: pageWidth - 40 });

    doc.font("Helvetica").fontSize(7.5).fillColor("#aec6d4")
        .text("Plot No. 123, Industrial Area, Phase-II, Ahmedabad, Gujarat — 382 445  |  Ph: +91 79 2583 XXXX", leftX + 20, headerY + 58, { width: pageWidth - 40 });
    ...
    doc.font("Helvetica").fontSize(8).fillColor(COLORS.textDark)
        .text(new Date().toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" }), leftX + colW * 2, detailY + 10);

    // Separator
    doc.moveTo(leftX, detailY + 26).lineTo(leftX + pageWidth, detailY + 26)
        .strokeColor(COLORS.accentGold).lineWidth(1).stroke();
...
function drawSubjectLine(doc: PDFKit.PDFDocument, data: CommercialOfferData): void {
    ...
    doc.font("Helvetica").fontSize(10).fillColor(COLORS.textDark)
        .text(`RFQ No: ${data.prNumber}   |   Company: ${data.companyName}   |   Location: ${data.location}`, leftX + 18, boxY + 30, { width: pageWidth - 30 });
...
function drawItemsTable(doc: PDFKit.PDFDocument, data: CommercialOfferData): void {
    ...
    const headers = ["Sl No.", "Item Code", "Item Name", "Material", "Qty", "Sale Price (₹)", "HSN Code", "GST %", "Total Before GST", "GST Amt", "Total with GST"];
    ...
    data.items.forEach((item, idx) => {
        ...
        cellTexts.forEach((text, i) => {
            ...
            doc.font(i === 0 ? "Helvetica-Bold" : "Helvetica").fontSize(7).fillColor(COLORS.textDark)
...
function drawTotals(doc: PDFKit.PDFDocument, data: CommercialOfferData): void {
    ...
    doc.font("Helvetica-Bold").fontSize(8).fillColor(COLORS.textDark)
        .text(`₹ ${formatCurrency(data.grandTotalBeforeGst)}`, boxX + 150, y + 5, { width: 120, align: "right" });
    y += 20;
    ...
    doc.font("Helvetica-Bold").fontSize(8).fillColor(COLORS.textDark)
        .text(`₹ ${formatCurrency(data.grandGstAmount)}`, boxX + 150, y + 5, { width: 120, align: "right" });
    y += 20;
    ...
    doc.font("Helvetica-Bold").fontSize(10).fillColor(COLORS.headerText)
        .text(`₹ ${formatCurrency(data.grandTotalWithGst)}`, boxX + 150, y + 6, { width: 120, align: "right" });
    y += 24;

    doc.y = y;
}

function drawFooter(doc: PDFKit.PDFDocument, data: CommercialOfferData): void {
    const pageWidth = pw(doc);
    const startX = m(doc).left;
    ...
    doc.font("Helvetica-Bold").fontSize(10).fillColor("#1e8449")
        .text(
            ...
            startX + 14, y + 10, { width: pageWidth - 28 }
        );
    ...
    doc.moveTo(startX, sigY + 40).lineTo(startX + 180, sigY + 40)
        .strokeColor(COLORS.textDark).lineWidth(0.5).stroke();
...
function calculateRowHeight(
    ...
    texts.forEach((text, i) => {
        const h = doc.font("Helvetica").fontSize(fontSize).heightOfString(text, { width: cols[i]! - 4 }) + 8;
        if (h > maxH) maxH = h;
    });
    return maxH;
}
